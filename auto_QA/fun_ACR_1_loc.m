...
I_bin=fun_add_threshold(I,mu/2);%half water mean as threshold
if visual==1%v3
    figure;
    imshow(I_bin,[]);
...
% text(p1(1)-50,(p1(2)-p2(2)),...%HW:50 pxls to left
%     [num2str(abs(distance_real_l)) '\rightarrow'],...%use 'normalized' to scale
%     'Color','r','FontUnits','normalized');      %letter to image
% 
...
% %     'Color','r','FontUnits','normalized');     %letter to image
% 
% text(p3(1)+3,(p3(2)-p4(2)),...%HW:3 pxls to left
%     ['\leftarrow' num2str(abs(distance_real_r))],...%use 'normalized' to scale
%     'Color','r','FontUnits','normalized');     %letter to image
